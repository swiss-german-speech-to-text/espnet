...
raw_data="downloads/bern_stadtrat"
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    find ${raw_data} -follow -name "*.wav" | sort | while read -r filename;do
    ...
    paste -d " " \
        <(cut -d "|" -f 1 < ${raw_data}/metadata.csv) \
        <(cut -d "|" -f 3 < ${raw_data}/metadata.csv) \
        > ${text}

...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    mkdir -p ${lmdatadir}
    cut -f 2- -d" " data/${train_set}/text | gzip -c > data/local/${lmdatadir}/${train_set}_text.gz
    cut -f 2- -d" " ${raw_data_europarl}/europarl-v7.de-en.de | gzip -c > data/local/${lmdatadir}/europarl_text.gz
    # combine external text and transcriptions and shuffle them with seed 777
    zcat data/local/lm_train/europarl_text.gz data/local/${lmdatadir}/${train_set}_text.gz |\
